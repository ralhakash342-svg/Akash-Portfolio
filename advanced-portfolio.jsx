...
import { ChevronDown, Menu, X, Github, Linkedin, Twitter, Mail, ExternalLink, Code2, Zap, Palette, Database, Cloud, CheckCircle2, ArrowRight } from 'lucide-react';
...
export default function AdvancedPortfolio() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (statsSection) {
        ...
        setVisibleStats(rect.top < window.innerHeight);
      }
    };
  ...
  const projects = [
    ...
    },
    {
      ...
      description: 'Advanced weather forecasting app with data visualization and historical analysis.',
      tech: ['Next.js', 'D3.js', 'OpenWeather API', 'PostgreSQL'],
      image: '🌦️',
      category: 'Frontend',
    },
    {
      title: 'DevOps Dashboard',
      description: 'Comprehensive DevOps monitoring platform with automated alerts and analytics.',
  ...
  const NavBar = () => (
    <nav className={`fixed top-0 w-full z-50 backdrop-blur-xl transition-all ${isDark ? 'bg-black/30 border-b border-white/10' : 'bg-white/30 border-b border-black/10'}`}>
      <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
        <div className={`text-2xl font-bold bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent animate-pulse`}>
          Dev.
        </div>
      ...
      <div className="h-1 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 transition-all" style={{ width: `${scrollProgress}%` }} />
    </nav>
  );
  ...
  const Hero = () => (
    <section id="hero" className={`min-h-screen flex items-center justify-center relative overflow-hidden pt-20 ${isDark ? 'bg-gradient-to-br from-black via-slate-900 to-black' : 'bg-gradient-to-br from-white via-slate-50 to-white'}`}>
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className={`absolute top-1/4 left-1/4 w-96 h-96 rounded-full opacity-20 blur-3xl animate-float ${isDark ? 'bg-blue-500' : 'bg-blue-300'}`} />
        <div className={`absolute bottom-1/4 right-1/4 w-96 h-96 rounded-full opacity-20 blur-3xl animate-float ${isDark ? 'bg-purple-500' : 'bg-purple-300'}`} style={{ animationDelay: '2s' }} />
        <div className={`absolute top-1/2 right-1/3 w-80 h-80 rounded-full opacity-20 blur-3xl animate-float ${isDark ? 'bg-pink-500' : 'bg-pink-300'}`} style={{ animationDelay: '4s' }} />
      </div>
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 text-center">
        ...
        <h1 className={`text-6xl md:text-8xl font-bold mb-6 ${isDark ? 'text-white' : 'text-black'}`}>
          Hi, I'm <span className="bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">Alex</span>
        </h1>
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-center mb-16">
          ...
          <button className={`px-8 py-3 rounded-lg font-semibold backdrop-blur-md transition-all hover:scale-105 group ${isDark ? 'bg-white/10 text-white border border-white/20 hover:bg-white/20' : 'bg-black/10 text-black border border-black/20 hover:bg-black/20'}`}>
        ...
        <div className="flex gap-4 justify-center mb-16">
          {[{ icon: Github, label: 'GitHub' }, { icon: Linkedin, label: 'LinkedIn' }, { icon: Twitter, label: 'Twitter' }, { icon: Mail, label: 'Email' }].map(({ icon: Icon, label }) => (
            <a key={label} href="#" className={`p-3 rounded-lg backdrop-blur-md transition-all hover:scale-110 hover:-translate-y-1 ${isDark ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-black/10 text-black hover:bg-black/20'}`}>
              <Icon size={20} />
            </a>
          ))}
        </div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map(({ icon: Icon, title, description }, i) => (
            <div 
              key={title} 
              className={`p-8 rounded-2xl backdrop-blur-md transition-all hover:scale-105 hover:shadow-xl ${isDark ? 'bg-white/5 border border-white/10 hover:bg-white/10' : 'bg-black/5 border border-black/10 hover:bg-black/10'}`}
    ...
    <section id="projects" className={`py-20 px-4 ${isDark ? 'bg-gradient-to-b from-black to-slate-900' : 'bg-gradient-to-b from-white to-slate-50'}`}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-1 lg:grid-cols-3 gap-6">
          {projects.map(({ title, description, tech, image, category }, idx) => (
            <div 
              key={title} 
              className={`rounded-2xl overflow-hidden backdrop-blur-md transition-all hover:scale-105 hover:shadow-2xl group ${isDark ? 'bg-white/5 border border-white/10' : 'bg-black/5 border border-black/10'}`}
              onMouseEnter={() => setHoveredProject(idx)}
              onMouseLeave={() => setHoveredProject(null)}
              style={{ animation: `fadeInUp 0.5s ease-out ${idx * 0.1}s both` }}
            >
              <div className={`text-6xl p-8 flex items-center justify-center h-48 transition-all ${isDark ? 'bg-gradient-to-br from-blue-900/20 to-purple-900/20' : 'bg-gradient-to-br from-blue-100 to-purple-100'} ${hoveredProject === idx ? 'scale-110' : ''}`}>
                {image}
              </div>
              <div className="p-6">
                <div className="flex justify-between items-start mb-2">
                  <h3 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-black'}`}>{title}</h3>
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${isDark ? 'bg-blue-500/20 text-blue-300' : 'bg-blue-200 text-blue-700'}`}>{category}</span>
                </div>
  ...
  const SkillBar = ({ name, level, isDark, delay }) => {
    return (
      <div style={{ animation: `fadeInUp 0.5s ease-out ${delay}s both` }}>
        ...
        <div className={`h-2 rounded-full overflow-hidden ${isDark ? 'bg-white/10' : 'bg-black/10'}`}>
          <div 
            className="h-full bg-gradient-to-r from-blue-500 to-purple-500 rounded-full transition-all duration-1000 origin-left"
            style={{ width: visibleStats ? `${level}%` : '0%' }}
          />
        </div>
      </div>
    );
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-4 gap-4 mb-16">
          {stats.map(({ label, value, icon }, i) => (
            <div 
              key={label} 
              className={`p-6 rounded-xl backdrop-blur-md text-center transition-all hover:scale-105 ${isDark ? 'bg-white/5 border border-white/10 hover:bg-white/10' : 'bg-black/5 border border-black/10 hover:bg-black/10'}`}
              style={{ animation: `fadeInUp 0.5s ease-out ${i * 0.1}s both` }}
            >
            ...
            <div key={category}>
              ...
              <div className="space-y-4">
                {items.map(({ name, level }, idx) => (
                  <SkillBar key={name} name={name} level={level} isDark={isDark} delay={catIdx * 0.1 + idx * 0.05} />
                ))}
              </div>
            </div>
          ))}
    ...
    <section id="contact" className={`py-20 px-4 ${isDark ? 'bg-gradient-to-b from-slate-900 to-black' : 'bg-gradient-to-b from-slate-50 to-white'}`}>
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-6 mt-16">
          <div className={`p-6 rounded-xl text-center backdrop-blur-md transition-all hover:scale-105 ${isDark ? 'bg-white/5 border border-white/10' : 'bg-black/5 border border-black/10'}`}>
  ...
  return (
    <div className={isDark ? 'bg-black text-white' : 'bg-white text-black'}>
      ...
      {scrollProgress > 20 && (
        <button
          onClick={scrollToTop}
          className="fixed bottom-8 right-8 p-3 rounded-lg bg-gradient-to-r from-blue-500 to-purple-500 text-white hover:shadow-lg transition-all hover:scale-110 z-40 animate-bounce"
        >
          ↑
        </button>
      )}
